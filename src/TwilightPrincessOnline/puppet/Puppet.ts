...
import TPOnline from '../../TwilightPrincessOnline';
import { Command, ICommandBuffer, ITPOnlineHelpers, RemoteSoundPlayRequest, TPOEvents } from '../api/TPOAPI';
...
export class Puppet {
  ...
  spawn(spawnIndex: number) {
    ...
    if (this.core.helper.isLinkControllable() && !this.isSpawned && !this.isSpawning && this.spawnHandle === undefined) {
      bus.emit(TPOEvents.PLAYER_PUPPET_PRESPAWN, this);
  ...
  despawn() {
    if (this.isSpawned) {
      ...
      bus.emit(TPOEvents.PLAYER_PUPPET_DESPAWNED, this);
    }
  }
}
